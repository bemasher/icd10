...
type NeoplasmNode struct {
	XMLName  xml.Name
	Content  string         `xml:",chardata"`
	Attrs    []xml.Attr     `xml:",attr,any"`
	Children []NeoplasmNode `xml:",any"`
...
func (n NeoplasmNode) Walk(ctx context.Context) chan util.AlphabeticTerm {
...
func (n NeoplasmNode) walk(ctx context.Context, terms chan util.AlphabeticTerm, depth int, breadcrumbs []string) {
	select {
	case <-ctx.Done():
		return
	default:
		...
		case "mainTerm", "term":
			...
			for idx, code := range n.Codes {
				...
				term.Title = title + ", " + neoplasmHeaders[idx]
